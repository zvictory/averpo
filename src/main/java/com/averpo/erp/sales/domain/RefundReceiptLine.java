...
/**
 * Пул қайтариш чеки сатри - credit_memo_line'нинг айнан кўзгуси
...
@Table(name = "refund_receipt_line",
       uniqueConstraints = @UniqueConstraint(columnNames = {"refund_receipt_id", "line_no"}))
@Getter
@NoArgsConstructor(access = AccessLevel.PROTECTED)
public class RefundReceiptLine extends BaseEntity {
    ...
    /** Gross (ҚҚСли) сумма - пул счёти кредити шу устида (amount + tax). */
    public BigDecimal grossAmount() {
        return amount.add(taxAmount);
    }

    /** Янги сатр - фақат RefundReceipt.addLine орқали (композиция). */
    RefundReceiptLine(RefundReceipt refundReceipt, int lineNo, InvoiceLineType type,
