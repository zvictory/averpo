...
Ҳолат: **ТАСДИҚЛАНГАН** (фойдаланувчи: «tasdiqlayman»).
...
Карталар: 101 очилди (бажарувчи кейин белгиланади -
...
## 1. Профиль саҳифаси (/profile) - DEC-101
...
- **Шахсий майдонлар** (фойдаланувчи кеч, ҳаммаси
  ...
  Choose-file/юклаш блоки эмас (жонли топилма).
    **Сайдбарда аватар** (sidebar.jte sbfoot): профиль менюси РАСМЛИ
  (аватар+ном, расм йўқ → placeholder) - 1-БОСҚИЧ (жонли топилма; аввал 2-босқич эди).
- **email admin томонидан ҳам** (жонли топилма): email
  профилда self-service, ЛЕКИН admin ҳам /users/{id}/edit'да киритади
  (ихтиёрий) - UserForm/UserController.
- **Бир марталик парол** (жонли топилма): admin парол
...
- **Bot API клиенти**: расмий `com.github.pengrad:java-telegram-bot-api`
  кутубхонаси (DEC-132) - кутубхона типлари `TelegramBotClient`
  портидан ташқарига чиқмайди (тест seam + lock-in изоляцияси).
  Модул: `com.averpo.erp.plugins.telegram.*`.
- **РЕЖИМ (профил асосида, DEC-138)**: Telegram янгиликларни икки
...
- **Webhook** (prod, DEC-138): `POST /telegram/webhook` (permitAll +
...
- **АРХИТЕКТУРА (фойдаланувчи)**: биз бир-tenant
  ...
  эмас - ихтиёрий фича ҳолати билан гейтланади). Карта: DEC-103.

## 4. App_user ↔ employee улаш (DEC-101 доирасида, changeset 057)

- Фойдаланувчи талаби: app_user'ни ходимга улаш имкони.
...
- **Танлагич = COMBOBOX (қидирувли)** (жонли топилма):
  мавжуд combobox (/js/combobox.js, DEC-066) - ходим кўп бўлса
