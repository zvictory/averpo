...
// формати билан бир хил - Arbitr-011), каср ажратгичи НУҚТА (терилган
// вергул нуқтага айланади), submit'да бўшлиқлар олиб ташланади -
// серверга тоза сон боради.
//
// Arbitr-135: window.averpoRateFmt - Fmt.rate'нинг ЯГОНА JS кўзгуси
...
(function () {
    ...
    // Курс КЎРСАТИШ формати (Fmt.rate кўзгуси, Arbitr-135): қиймат >= 1
...
//
// Мини-калькулятор (Arbitr-064, spec: ui-navigation-display.md W):
...
(function () {
    ...
    // хато ҳисобланади. Курс майдонида (data-rate-field, Arbitr-135)
    ...
    function applyCalc(el) {
        ...
        if (r !== null) {
            ...
            if (v && v.indexOf('e') < 0 && v.indexOf('E') < 0) {
                el.value = v;
                reformat(el);
                // Arbitr-086: программатик .value ёзуви input чиқармайди -
    ...
    // Arbitr-162: HTMX partial (drawer таҳрири, payroll prefill сатрлари)
...
// Arbitr-127: HTMX partial сўровида server хатоси (4xx/5xx). htmx 2
